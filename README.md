...
> **Status: pre-launch cleanup pass.** This repository is being handed off
> to the IT team to deploy to production at **https://drumwave.com/**.
> Content, code, SEO, analytics and the contact form have all been reviewed
> and reorganized — see [Audit reports](#audit-reports) below for the full
...
- **Privacy Policy** ([`privacy-policy.html`](privacy-policy.html), EN) and
  **Política de Privacidade**
  ([`politica-de-privacidade.html`](politica-de-privacidade.html), PT-BR /
  LGPD) — these are translations of each other, not duplicates; both are
  linked from every footer and cross-referenced via `hreflang`.
...
politica-de-privacidade.html   Privacy policy (PT-BR / LGPD)
...
See [`docs/CONTACT_FORM_SECURITY.md`](docs/CONTACT_FORM_SECURITY.md) for
the full setup details and current deployment status, and
...
- **Contact form live end-to-end test** — the reCAPTCHA proxy (Lambda + API
  Gateway) is deployed and verified via direct API calls, but still needs a
  real submission test from the published site to confirm the full flow
  reaches HubSpot — see `docs/CONTACT_FORM_SECURITY.md`.
